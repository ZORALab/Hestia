...
the attributes listed in the Switches section.`
		...
		// prepare
		subject := create_number_string(s)
		subject = configure_float64_string(s, subject)
		hestiaTESTING.Log(s,
...
func assert_S64_ParseFLOAT64_output(s *hestiaTESTING.Scenario, output float64) bool {
	...
	case hestiaTESTING.HasCondition(s, cond_VALUE_FLOAT64):
	...
	case hestiaTESTING.HasCondition(s, cond_NOTATION_ISO6093NR3):
		return assert_S64_ParseFLOAT64_output_iso7093nr3(s, output)
	}
...
func assert_S64_ParseFLOAT64_output_iso7093nr3(s *hestiaTESTING.Scenario, output float64) bool {
	switch {
...
func configure_float64_string(s *hestiaTESTING.Scenario, subject string) string {
	if hestiaTESTING.HasCondition(s, cond_VALUE_FLOAT64) {
		return configure_float64_minmax_string(s, subject)
	}

	if hestiaTESTING.HasCondition(s, cond_NOTATION_IEEE754) {
		return configure_float64_ieee754_string(s, subject)
	}

	return subject
}

func configure_float64_minmax_string(s *hestiaTESTING.Scenario, subject string) (out string) {
	switch {
	case hestiaTESTING.HasCondition(s, cond_NEGATIVE):
		out = "-4.9406564584124654417656879286822137236505980"
	case hestiaTESTING.HasCondition(s, cond_POSITIVE):
		out = "1.79769313486231570814527423731704356798070"
	default:
		return subject
	}
...
func configure_float64_ieee754_string(s *hestiaTESTING.Scenario, subject string) string {
	switch {
	case hestiaTESTING.HasCondition(s, cond_BROKEN):
