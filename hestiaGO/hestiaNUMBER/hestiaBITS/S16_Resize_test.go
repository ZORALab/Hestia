...
func test_cases_S16_Resize() []*hestiaTESTING.Scenario {
...
func Test_S16_Resize(t *testing.T) {
	scenarios := test_cases_S16_Resize()
	...
	for i, s := range scenarios {
		...
		if !assert_S16_Resize_output(s, subject) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}

		if !assert_S16_Resize_error(s, err) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}
...
func assert_S16_Resize_output(s *hestiaTESTING.Scenario, output uint16) bool {
	...
	if hestiaTESTING.HasCondition(s, cond_TO_BITS_12) {
		return output == uint16(value_MASKED_BITS_12)
	}
	...
	if hestiaTESTING.HasCondition(s, cond_TO_BITS_5) {
		return output == uint16(value_MASKED_BITS_5)
	}
