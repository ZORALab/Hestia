...
func test_cases_S16_TrailingZeros() []*hestiaTESTING.Scenario {
	return []*hestiaTESTING.Scenario{
		{
			Description: `
test hestiaNUMBER/hestiaBITS/S16_TrailingZeros is able to process 0-bits value.
`,
			Switches: []string{
				cond_BITS_0,
			},
			...
			Description: `
test hestiaNUMBER/hestiaBITS/S16_TrailingZeros is able to process 8-bits value.
`,
			Switches: []string{
				cond_BITS_8,
			},
			...
			Description: `
test hestiaNUMBER/hestiaBITS/S16_TrailingZeros is able to process 16-bits value.
`,
			Switches: []string{
				cond_BITS_16,
			},
...
func Test_S16_TrailingZeros(t *testing.T) {
	scenarios := test_cases_S16_TrailingZeros()

	for i, s := range scenarios {
		s.ID = uint64(i)
		s.Name = "hestiaNUMBER/hestiaBITS/S16_TrailingZeros API"
		...
		if !assert_S16_TrailingZeros_output(s, output) {
...
func assert_S16_TrailingZeros_output(s *hestiaTESTING.Scenario, output uint16) bool {
