...
fn test_conclusion_algorithm(id: u64, desc: String, switches: Vec<String>) {
	// prepare
...
// test suites
hestia_testing_exec!(test_conclusion_unknown_verdict, {
	test_conclusion_algorithm(
		3,
		"\
test hestia_testing::conclusion() is able to return unknown verdict if set corrupted.
"
		.to_string(),
		vec![
...
hestia_testing_exec!(test_conclusion_skip_verdict, {
	test_conclusion_algorithm(
		2,
		"\
test hestia_testing::conclusion() is able to return hestia_testing::VERDICT_SKIP.
"
		.to_string(),
		vec![
			String::from(testlibs_test::COND_PROPER_NAME),
...
hestia_testing_exec!(test_conclusion_fail_verdict, {
	test_conclusion_algorithm(
		1,
		"\
test hestia_testing::conclusion() is able to return hestia_testing::VERDICT_FAIL.
"
		.to_string(),
		vec![
			String::from(testlibs_test::COND_PROPER_NAME),
...
hestia_testing_exec!(test_conclusion_proper_verdict, {
	test_conclusion_algorithm(
		0,
		"\
test hestia_testing::conclusion() is able to return hestia_testing::VERDICT_PASS.
"
		.to_string(),
		vec![
			String::from(testlibs_test::COND_PROPER_NAME),
